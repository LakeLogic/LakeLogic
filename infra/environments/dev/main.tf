...
module "vnet" {
  ...
  bastion_subnet_prefix          = var.networking_config.bastion_subnet_prefix
  enable_bastion                 = var.enable_bastion  # Control Bastion deployment
}
...
module "frontend" {
  ...
  prefix              = local.prefix
  location            = "westeurope"  # UK South not supported for Static Web Apps
  resource_group_name = module.vnet.resource_group_name
  sku_tier            = var.swa_sku
  sku_size            = var.swa_sku
}

# 7. Windows Jump Server (Database Admin with GUI)
module "windows_jumpbox" {
  source              = "../../modules/windows-jumpbox"
  prefix              = local.prefix
  location            = var.location
  resource_group_name = module.vnet.resource_group_name
  subnet_id           = module.vnet.private_subnet_id  # Use private endpoint subnet
  postgres_host       = module.db.db_hostname
  
  admin_username = var.jumpbox_admin_username
  admin_password = var.jumpbox_admin_password
  vm_size        = var.jumpbox_vm_size
  allowed_rdp_ip = var.jumpbox_allowed_rdp_ip

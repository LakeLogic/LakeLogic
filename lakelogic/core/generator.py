...
class DataGenerator:
    ...
    def generate_related(
        ...
        window_start, window_end : datetime, optional
            Fix the generation time window for **all** temporal columns across **every**
            entity. Without this, timestamp columns default to ``datetime.now()`` and are
            therefore not reproducible from ``seed`` — pass a fixed window to make the whole
            related product (timestamps included) reproducible from ``(seed, window)``.
        unique_entity_seeds : bool, default False
            Opt in (``True``) to derive a distinct per-entity seed from
            ``(seed, entity_name)`` so two entities with an identical schema do not generate
            byte-identical rows (incl. identical primary keys) under one shared ``seed`` —
        ...
        def _entity_seed(base: Optional[int], entity: str) -> Optional[int]:
            """Deterministic per-entity seed derived from ``(base, entity_name)``.

            With one shared ``seed`` every entity's generator produced the same stream, so
            two entities with an *identical* schema generated byte-identical rows (incl.
            identical primary keys) — which lets a child FK satisfy referential integrity
            against the wrong parent. Salting by entity name (``zlib.crc32``, process-stable)
            keeps generation reproducible per entity while making distinct entities distinct.
            ``None`` (no seed ⇒ non-deterministic) is preserved as ``None``.
            """
            if base is None:
                return None
            import zlib
        ...
        for name, schema_source in contracts.items():
            entity_seed = _entity_seed(seed, name) if unique_entity_seeds else seed
            gen = DataGenerator(schema_source, seed=entity_seed, use_faker=use_faker)
            generators[name] = gen

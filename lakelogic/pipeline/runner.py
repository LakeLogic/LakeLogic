...
class LakehousePipeline:
    ...
    def _execute_resets(self, active_contracts: List[RegistryContract], reset_layers: Set[str], reload_layers: Set[str], dry_run: bool) -> None:
    ...
    def _execute_gdpr_pass(self, active_contracts: List[RegistryContract], subject_col: str, subject_ids: List[str], strategy: str, salt: str, dry_run: bool, partition_filter: Optional[Dict[str, str]] = None):
        ...
        logger.info(f"GDPR Erasure Pass: {len(subject_ids)} subjects on '{subject_col}' (Strategy: {strategy}){partition_msg}")
        ...
        for c in active_contracts:
            ...
            if mat_target.startswith("table:") and self.spark:
                ...
                if affected > 0 or dry_run:
                    report = generate_erasure_report(dc, subject_col, subject_ids, strategy, affected, partition_filter=partition_filter)
                ...
                logger.warning(f"Path-based target for [{c.layer}] {c.entity} not yet supported in native driver erasure pass.")


    def _execute_hipaa_pass(self, active_contracts: List[RegistryContract], patient_col: str, patient_ids: List[str], strategy: str, salt: str, dry_run: bool, partition_filter: Optional[Dict[str, str]] = None):
        ...
        if partition_filter:
            partition_msg = f" [partition: {partition_filter['column']}='{partition_filter['value']}']"
        logger.info(f"HIPAA Erasure Pass: {len(patient_ids)} patients on '{patient_col}' (Strategy: {strategy}){partition_msg}")
        ...
        for c in active_contracts:
            ...
            if mat_target.startswith("table:") and self.spark:
                ...
                if affected > 0 or dry_run:
                    report = generate_hipaa_erasure_report(dc, patient_col, patient_ids, strategy, affected, partition_filter=partition_filter)
                    report["pipeline_run_id"] = self.run_id
    ...
    def run(
        ...
        *,
        ...
        targets = [l.strip().lower() for l in target_layers.split(",") if l.strip()]
        layer_order = ["bronze", "silver", "gold"]
        target_set = set(layer_order) if "all" in targets else set(targets)
        
        resets = {l.strip().lower() for l in reset_layers.split(",") if l.strip()}
        reloads = {l.strip().lower() for l in reload_layers.split(",") if l.strip()}
        ...
        if forget_column and forget_values:
            gdpr_partition = {"column": forget_partition_column, "value": forget_partition_value} if forget_partition_column else None
            self._execute_gdpr_pass(all_active, forget_column, forget_values, forget_strategy, forget_salt, dry_run, partition_filter=gdpr_partition)
            
        if forget_patient_column and forget_patient_ids:
            hipaa_partition = {"column": forget_patient_partition_column, "value": forget_patient_partition_value} if forget_patient_partition_column else None
            self._execute_hipaa_pass(all_active, forget_patient_column, forget_patient_ids, forget_patient_strategy, forget_patient_salt, dry_run, partition_filter=hipaa_partition)
                ...
                try:
                    ...
                    if self.engine == "spark" and not hasattr(df_good, "sparkSession"):
                        ...
                        if isinstance(df_good, pl.DataFrame):
                            void_cols = [col for col, t in zip(df_good.columns, df_good.dtypes) if str(t) in ("Null", "null", "Void", "void")]
                            if void_cols:
                        ...
                        if df_bad is not None and not hasattr(df_bad, "sparkSession"):
                            if isinstance(df_bad, pl.DataFrame):
                                void_cols = [col for col, t in zip(df_bad.columns, df_bad.dtypes) if str(t) in ("Null", "null", "Void", "void")]
                                if void_cols:
                    ...
                    row_count = len(df_good) if hasattr(df_good, "__len__") else (df_good.count() if hasattr(df_good, "count") else "?")

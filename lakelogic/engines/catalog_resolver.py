...
Timeout Behaviour
~~~~~~~~~~~~~~~~~
The Databricks SDK's default credential chain can block the calling thread
indefinitely when it falls back to interactive browser auth or an unreachable
Azure Instance Metadata Service (IMDS) endpoint.  To prevent pipeline hangs,
all SDK calls are wrapped in a ``concurrent.futures`` thread pool with a
configurable timeout (default 60 s, override via ``UC_RESOLVE_TIMEOUT_SECONDS``
env var).  When the timeout fires a ``TimeoutError`` is raised immediately.
...
# Timeout (seconds) for Databricks SDK calls.  The SDK's default credential
# chain can silently block forever when it falls back to interactive browser
# auth or an unreachable Azure IMDS endpoint.  This safeguard ensures the
# pipeline fails fast instead of hanging indefinitely.
UC_RESOLVE_TIMEOUT_SECONDS: int = int(os.getenv("UC_RESOLVE_TIMEOUT_SECONDS", "60"))
...
class UnityCatalogResolver:
    ...
    def _resolve_with_sdk(self, table_name: str) -> str:
        ...
        The SDK call is dispatched to a background thread with a
        ``UC_RESOLVE_TIMEOUT_SECONDS`` guard (default 60 s) so that the
        main pipeline thread is never blocked by slow credential discovery.
        ...
        Raises:
            ImportError: If Databricks SDK is not installed
            TimeoutError: If the SDK call exceeds the configured timeout
        """
        try:
        ...
        def _sdk_resolve() -> str:
            """Inner function executed in a worker thread."""
        ...
        timeout = UC_RESOLVE_TIMEOUT_SECONDS
        logger.debug(f"Resolving UC table {table_name} via Databricks SDK (timeout={timeout}s)")

        try:
            with ThreadPoolExecutor(max_workers=1) as pool:
                future = pool.submit(_sdk_resolve)
                return future.result(timeout=timeout)
        except FuturesTimeoutError:
            raise TimeoutError(
                f"Unity Catalog resolution for '{table_name}' timed out after "
                f"{timeout}s.  The Databricks SDK credential chain may be "
                f"blocked (e.g. interactive browser auth or unreachable IMDS "
                f"endpoint).  Set DATABRICKS_HOST and DATABRICKS_TOKEN "
                f"explicitly, or increase UC_RESOLVE_TIMEOUT_SECONDS."
            )
...
def get_unity_catalog_resolver() -> UnityCatalogResolver:
    ...
    Example:
        >>> from lakelogic.engines.catalog_resolver import get_unity_catalog_resolver

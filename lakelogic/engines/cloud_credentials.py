...
class CloudCredentialResolver:
    ...
    def resolve_storage_options(self, path: str, storage_options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
    ...
    def _get_sp_token(self, scope: str = "https://storage.azure.com/.default") -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"✅ Azure service-principal authentication successful (client_id={self._azure_client_id})")
            return token.token
    ...
    def _get_azure_key_vault_secret(
        ...
        try:
            ...
            if not vault_url:
                logger.warning("Azure Key Vault URL not provided. Pass vault_url or set AZURE_KEY_VAULT_URL.")
                return None
            ...
            if self._azure_client_id and self._azure_client_secret and self._azure_tenant_id:
                ...
                logger.debug(f"Key Vault: using service-principal credential (client_id={self._azure_client_id})")
            else:
                from azure.identity import DefaultAzureCredential
    ...
    def _get_aws_secret(self, secret_name: str, region: Optional[str] = None) -> Optional[str]:
    ...
    def _get_gcp_secret(self, secret_name: str, project_id: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(
                "GCP Secret Manager library not installed. Install with: pip install google-cloud-secret-manager"
            )
            return None
        except Exception as e:
    ...
    **All three using default scope names:**
    ...
    def for_cloud(
        ...
        if cloud not in cls._DEFAULT_SCOPES:
            raise ValueError(f"Unknown cloud {cloud!r}. Valid options: {list(cls._DEFAULT_SCOPES)}")
    ...
    def _build_azure(dbu, scope: str) -> "CloudCredentialResolver":
        ...
        logger.info(f"DatabricksSecretResolver: loaded Azure SP credentials (scope={scope!r}, client_id={client_id})")
    ...
    def _build_aws(dbu, scope: str) -> "CloudCredentialResolver":
        ...
        logger.info(f"DatabricksSecretResolver: loaded AWS credentials from scope={scope!r} into environment")
    ...
    def _build_gcp(dbu, scope: str) -> "CloudCredentialResolver":
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False, prefix="lakelogic_sa_") as f:
            f.write(sa_json)
            sa_path = f.name
        ...
        logger.info(f"DatabricksSecretResolver: loaded GCP service account (scope={scope!r}, project={project})")

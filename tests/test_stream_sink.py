...
class TestAvailableNow:
    def test_drains_all_records(self, tmp_path):
        ...
        proc = _FakeProcessor()
        sink = StreamSink(
            source=_events(250, bad_every=10), processor=proc, checkpoint=store, checkpoint_key="k", batch_size=100
        )
    ...
    def test_checkpoint_committed_after_run(self, tmp_path):
        store = SQLiteCheckpointStore(tmp_path / "ck.sqlite")
        StreamSink(
            source=_events(50), processor=_FakeProcessor(), checkpoint=store, checkpoint_key="k", batch_size=25
        ).run("available_now")
        ck = store.load("k")
...
class TestResume:
    def test_second_run_resumes_and_processes_nothing_new(self, tmp_path):
        store = SQLiteCheckpointStore(tmp_path / "ck.sqlite")
        data = _events(120)
        StreamSink(source=data, processor=_FakeProcessor(), checkpoint=store, checkpoint_key="k", batch_size=50).run(
            "available_now"
        )
        # Same bounded source, re-run: cursor==120 already, nothing left.
        proc2 = _FakeProcessor()
        s2 = StreamSink(source=data, processor=proc2, checkpoint=store, checkpoint_key="k", batch_size=50).run(
            "available_now"
        )
    ...
    def test_crash_before_commit_resumes_with_no_gap_no_dup(self, tmp_path):
        ...
        with pytest.raises(RuntimeError, match="injected write failure"):
            StreamSink(source=data, processor=proc1, checkpoint=store, checkpoint_key="k", batch_size=100).run(
                "available_now"
            )
        ...
        proc2 = _FakeProcessor()
        s2 = StreamSink(source=data, processor=proc2, checkpoint=store, checkpoint_key="k", batch_size=100).run(
            "available_now"
        )
    ...
    def test_commit_failure_reprocesses_at_least_once(self, tmp_path):
        ...
        proc2 = _FakeProcessor()
        StreamSink(source=data, processor=proc2, checkpoint=store, checkpoint_key="k", batch_size=100).run(
            "available_now"
        )
...
class TestConnectorAndModes:
    ...
    def test_cursor_fn_used_for_checkpoint(self, tmp_path):
        ...
        StreamSink(
            ...
            checkpoint=store,
            checkpoint_key="k",
            cursor_fn=lambda e: {"offset": e["id"]},
        ).run("available_now")
    ...
    def test_default_checkpoint_key_from_contract_dataset(self, tmp_path):
        ...
        sink = StreamSink(
            source=[], processor=_FakeProcessor(), checkpoint=SQLiteCheckpointStore(tmp_path / "ck.sqlite")
        )
    ...
    def test_available_now_respects_max_batches(self, tmp_path):
        ...
        s = StreamSink(
            ...
            checkpoint=SQLiteCheckpointStore(tmp_path / "ck.sqlite"),
            checkpoint_key="k",
        ).run("available_now", max_batches=4)
...
class TestKafkaOffsetResume:
    def test_drains_all_partitions_and_records_offsets(self, tmp_path):
        ...
        store = SQLiteCheckpointStore(tmp_path / "ck.sqlite")
        s = StreamSink(
            source=src, processor=_FakeProcessor(), checkpoint=store, checkpoint_key="k", batch_size=100
        ).run("available_now")
    ...
    def test_resume_seeks_to_committed_offsets_no_reprocess(self, tmp_path):
        ...
        proc1 = _FakeProcessor()
        StreamSink(
            source=_kafka_source(data), processor=proc1, checkpoint=store, checkpoint_key="k", batch_size=100
        ).run("available_now")
        ...
        proc2 = _FakeProcessor()
        s2 = StreamSink(
            source=_kafka_source(data), processor=proc2, checkpoint=store, checkpoint_key="k", batch_size=100
        ).run("available_now")
    ...
    def test_new_records_after_resume_are_consumed(self, tmp_path):
        ...
        StreamSink(
            source=_kafka_source({0: _events(100)}),
            processor=_FakeProcessor(),
            checkpoint=store,
            checkpoint_key="k",
            batch_size=100,
        ...
        proc2 = _FakeProcessor()
        s2 = StreamSink(
            source=_kafka_source({0: _events(160)}),
            processor=proc2,
            checkpoint=store,
            checkpoint_key="k",
            batch_size=100,
    ...
    def test_paging_respects_max_poll_records(self, tmp_path):
        ...
        store = SQLiteCheckpointStore(tmp_path / "ck.sqlite")
        s = StreamSink(
            source=_kafka_source({0: _events(120)}),
            processor=_FakeProcessor(),
            checkpoint=store,
            checkpoint_key="k",
            batch_size=100,
    ...
    def test_continuous_offset_aware_bounded_by_max_batches(self, tmp_path):
        ...
        store = SQLiteCheckpointStore(tmp_path / "ck.sqlite")
        s = StreamSink(
            source=_kafka_source({0: _events(300)}),
            processor=_FakeProcessor(),
            checkpoint=store,
            checkpoint_key="k",
            batch_size=100,
        ).run("continuous", max_batches=2)
    ...
    def test_decode_bytes_json_deserializer_and_passthrough(self):
        ...
        src2 = KafkaOffsetSource(
            "t", consumer=object(), tp_factory=_SimpleTP, value_deserializer=lambda b: {"raw": b.decode()}
        )
...
class TestSSEOffsetResume:
    def test_drains_feed_and_records_last_event_id(self, tmp_path):
        ...
        store = SQLiteCheckpointStore(tmp_path / "ck.sqlite")
        s = StreamSink(source=src, processor=_FakeProcessor(), checkpoint=store, checkpoint_key="k", batch_size=20).run(
            "available_now"
        )
    ...
    def test_resume_replays_after_last_event_id_no_reprocess(self, tmp_path):
        ...
        proc1 = _FakeProcessor()
        StreamSink(
            source=SSEOffsetSource(connect=_sse_feed(events)),
            processor=proc1,
            checkpoint=store,
            checkpoint_key="k",
            batch_size=40,
    ...
    def test_new_events_after_resume_are_consumed(self, tmp_path):
        ...
        StreamSink(
            source=SSEOffsetSource(connect=_sse_feed(first)),
            processor=_FakeProcessor(),
            checkpoint=store,
            checkpoint_key="k",
            batch_size=100,
        ...
        proc2 = _FakeProcessor()
        s2 = StreamSink(
            source=SSEOffsetSource(connect=_sse_feed(grown)),
            processor=proc2,
            checkpoint=store,
            checkpoint_key="k",
            batch_size=100,
    ...
    def test_keepalive_comment_lines_skipped(self, tmp_path):
        ...
        store = SQLiteCheckpointStore(tmp_path / "ck.sqlite")
        s = StreamSink(
            source=SSEOffsetSource(connect=connect),
            processor=_FakeProcessor(),
            checkpoint=store,
            checkpoint_key="k",
            batch_size=100,
...
def _keyset_fetch(rows):
    ...
    def fetch(after, limit):
        start = 0 if after is None else next((i for i, r in enumerate(table) if r["wm"] > after), len(table))
...
class TestWatermarkChunkSource:
    ...
    def test_resume_from_committed_watermark_no_reprocess(self, tmp_path):
        ...
        StreamSink(
            source=WatermarkChunkSource(_keyset_fetch(rows), watermark_field="wm", chunk_size=50),
            processor=_FakeProcessor(),
            checkpoint=store,
            checkpoint_key="k",
            batch_size=50,
    ...
    def test_new_rows_after_resume_are_paged(self, tmp_path):
        ...
        StreamSink(
            source=WatermarkChunkSource(_keyset_fetch(first), watermark_field="wm", chunk_size=100),
            processor=_FakeProcessor(),
            checkpoint=store,
            checkpoint_key="k",
            batch_size=100,
        ...
        proc2 = _FakeProcessor()
        s2 = StreamSink(
            source=WatermarkChunkSource(_keyset_fetch(grown), watermark_field="wm", chunk_size=100),
            processor=proc2,
            checkpoint=store,
            checkpoint_key="k",
            batch_size=100,
    ...
    def test_crash_before_commit_resumes_from_watermark(self, tmp_path):
        ...
        with pytest.raises(RuntimeError, match="injected write failure"):
            StreamSink(
                source=WatermarkChunkSource(_keyset_fetch(rows), watermark_field="wm", chunk_size=100),
                processor=proc1,
                checkpoint=store,
                checkpoint_key="k",
                batch_size=100,
    ...
    def test_short_final_page_drains(self, tmp_path):
        ...
        s = StreamSink(
            source=WatermarkChunkSource(_keyset_fetch(rows), watermark_field="wm", chunk_size=10),
            processor=_FakeProcessor(),
            checkpoint=SQLiteCheckpointStore(tmp_path / "ck.sqlite"),
            checkpoint_key="k",
            batch_size=1000,
...
class TestSparkStreamSink:
    def test_requires_checkpoint_location(self):
        with pytest.raises(ValueError, match="checkpoint_location is required"):
            SparkStreamSink(stream_df=_FakeStreamDF([]), processor=_FakeProcessor(), checkpoint_location=None)
    ...
    def test_on_batch_hook_fires_per_micro_batch(self):
        ...
        SparkStreamSink(
            ...
            processor=_FakeProcessor(),
            checkpoint_location="/chk",
            on_batch=lambda b: seen.append(b.batch_id),
        ).run()
    ...
    def test_invalid_trigger_rejected(self):
        with pytest.raises(ValueError, match="trigger must be"):
            SparkStreamSink(
                stream_df=_FakeStreamDF([]), processor=_FakeProcessor(), checkpoint_location="/chk", trigger="firehose"
            )
    ...
    def test_await_termination_override(self):
        ...
        q2 = SparkStreamSink(stream_df=df2, processor=_FakeProcessor(), checkpoint_location="/chk").run(

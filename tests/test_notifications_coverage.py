...
def test_vault_cached_and_exception(monkeypatch):
    ...
    class FakeClient:
        def __init__(self, **kwargs):
            self.secrets = types.SimpleNamespace(
                kv=types.SimpleNamespace(v1=types.SimpleNamespace(read_secret=self.read_secret_v1))
            )
    ...
    # Test cached
    assert (
        nb._resolve_vault_secret("my-secret#myfield", {"vault_url": "http://vault", "vault_kv_version": "1"})
        == "vault-value"
    )
...
def test_local_secrets(tmp_path, monkeypatch):
    ...
    class FakeFernetError(Exception):
        pass

    class FakeFernet:
        def __init__(self, key):
            if key == b"bad-key":
                raise FakeFernetError()
    ...
    res2 = nb._resolve_local_secret(
        "my-secret", {"secrets_file": "secrets.enc", "secrets_key": "good-key", "_base_path": str(tmp_path)}
    )
...
def test_console_adapter_encode_error(monkeypatch, capsys):
    class BadStdout:
        def __init__(self):
            self.buffer = types.SimpleNamespace(write=lambda b: None, flush=lambda: None)

...
- Update changelog for v1.66.0

### Fixed

- **materialization**: Keep the scd1 unknown member's key so it is injected once
## [1.66.0] — 2026-09-19
...
- Update deletion vector handling and improve compatibility with deltalake and polars
## [1.57.0] — 2026-09-10
...
- **DataProcessor**: Add contract_path to handle source path translation
## [1.52.1] — 2026-09-07

### Changed

- **SLOValidator**: Remove redundant storage variable assignments
## [1.52.0] — 2026-09-06
...
- **anomaly-detection**: Implement row count anomaly detection and configuration
- **run-log**: Record a traceback and version with every failure
- **telemetry**: Send the traceback and version to the platform

### Changed

- **logs**: Halve the run-complete line so it fits one row
- **registry**: Storage_mode names the addressing scheme, not a vendor

### Documentation

- Update changelog for v1.50.0

### Fixed

- **float-precision**: Unify float type across engines to 32-bit for consistency
- **models**: Stop warning about the ownership block the loader injects
- **runner**: Page the contract's own owner, not just the domain's
## [1.50.0] — 2026-09-05
...
- Update changelog for v1.47.0

### Fixed

- **deps**: Declare SQLAlchemy, which every database source already required
## [1.47.0] — 2026-09-02

### Added

- **ingestion**: Native CDC, declarative SFTP, and SQL transformations
- **sftp**: Deliver to an SFTP target from the contract
- **warehouse**: Keep Snowflake/BigQuery results in the warehouse

### CI/CD

- **notebooks**: Execute every example notebook on a clean Linux runner

### Documentation

- Governed data product blueprint catalogue — plan and first example
- Update changelog for v1.46.0

### Fixed

- **delta**: Route Delta reads around polars' broken bridge
- **engines**: Honour json_extract, nested types and rename on every engine
- **examples**: Repair the notebook gallery and guard it against API drift
- **release**: Pin release.bat to its own repo
- **sftp**: Declare asyncssh, and implement the incremental extraction it claimed

### Testing

- Enhance Delta read tests and improve FakeSelectableDataFrame to support dtypes
## [1.46.0] — 2026-08-29
...
[1.41.0]: https://github.com/lakelogic/LakeLogic/compare/v1.41.1...v1.41.0
[1.41.1]: https://github.com/lakelogic/LakeLogic/compare/v1.40.6...v1.41.1

...
def create_observatory_view():
    ...
    if all_logs:
        ...
        pdf_logs = df_logs.select([
            "started_at", "domain", "system", "entity", "layer", "status", "target_records", "quarantined_records", "cost_usd"
        ]).sort("started_at", descending=True).head(50).to_pandas()

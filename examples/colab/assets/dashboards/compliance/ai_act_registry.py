...
def create_ai_registry_view():
    ...
    kpi_cards = pn.Row(
        ...
        pn.indicators.Number(name="High Risk Systems", value=high_risk, format="{value}", colors=[(1, "red")] if high_risk > 0 else [(1, "green")]),
    ...
    body = pn.Column(
        ...
        pn.pane.Markdown("This live registry auto-discovers all LakeLogic data contracts globally tagged with `eu_ai_act` metadata. It tracks bias-examination, human-oversight tooling, and risk categorization across the mesh."),
        kpi_cards,

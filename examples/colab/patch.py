import json
with open('C:\\_Personal\\_SaaS\\lakelogic\\examples\\colab\\07_dlt_prefect_pipeline.ipynb', 'r', encoding='utf-8') as f:
    nb = json.load(f)
    ...
    if cell.get('cell_type') == 'code':
        ...
        for i, line in enumerate(source):
            if "with open('07_dlt_prefect_pipeline/_registry.yaml', 'w')" in line:
                ...
                new_source.append("registry = DomainRegistry.from_yaml('07_dlt_prefect_pipeline/_registry.yaml', storage_mode='direct')\n")
                new_source.append("\n")
                new_source.append("print(f\"✅ Registry loaded: {len(registry.contracts)} contracts across {registry.domain}/{registry.system}\")\n")
    ...
    if cell.get('cell_type') == 'markdown':
        for i, line in enumerate(cell.get('source', [])):
            if "`%pip install dlt" in line:
                cell['source'][i] = line.replace("`%pip install dlt", "`%pip install dlt") # wait, the lint says use `%pip install` instead of `!pip install`. Did they say `!pip install` before?
            if "`!pip install dlt" in line:
                cell['source'][i] = line.replace("`!pip install dlt", "`%pip install dlt")

with open('C:\\_Personal\\_SaaS\\lakelogic\\examples\\colab\\07_dlt_prefect_pipeline.ipynb', 'w', encoding='utf-8') as f:
